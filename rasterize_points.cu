	...
	const bool render_depth)
{
  ...
  return std::make_tuple(rendered, num_buckets, out_color, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer, metricCount, out_invdepth);
}
	...
	const bool render_depth) 
{
  ...
  if(render_depth && dL_dout_invdepth.size(0) != 0)
  {
