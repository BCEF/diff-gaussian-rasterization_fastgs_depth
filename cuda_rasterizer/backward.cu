	...
	float* dL_dcov)
{
	...
	if (dL_dinvdepth != nullptr)
		dL_dtz += dL_dinvdepth[idx] * (-1.f * tz2);
...
PerGaussianRenderCUDA(
	...
	float Register_dL_dmean2D_z = 0.0f;  //abs
	float Register_dL_dmean2D_w = 0.0f;  //abs
...
renderCUDA(
	...
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_dinvdepths,
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dinvdepth,
	...
	bool render_depth)
{
	...
	float accum_rec[C] = { 0 };
	float dL_dpixel[C];
	float dL_dinvdepth_pix = 0;
	float accum_rec_invdepth = 0;
	if (inside) {
		for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
		if (render_depth)
			dL_dinvdepth_pix = dL_dinvdepths[pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (render_depth) {
				const float invd = 1.0f / depths[global_id];
				accum_rec_invdepth = last_alpha * last_invdepth + (1.f - last_alpha) * accum_rec_invdepth;
				last_invdepth = invd;
				dL_dalpha += (invd - accum_rec_invdepth) * dL_dinvdepth_pix;
				atomicAdd(&(dL_dinvdepth[global_id]), dchannel_dcolor * dL_dinvdepth_pix);
			}
			...
			if (render_depth) {
				// background invdepth is 0
				dL_dalpha += (-T_final / (1.f - alpha)) * 0.0f * dL_dinvdepth_pix;
			}
